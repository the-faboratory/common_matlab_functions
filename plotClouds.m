...
nCells = size(cellArrayOfData,2) % Number of cells
nCol = size(cellArrayOfData{1},2) % Number of columns
allDataF = zeros(0,nCol);   % initialize AllData array
% Concatenate data
for i = 1:1
    allDataF = vertcat(allDataF, cellArrayOfData{i}(:,:));      % concatenate each sensor i
end
allDataSorted = sortrows(allDataF, xColumn); % sort data 
sensorData_FirstCell = cellArrayOfData{1}(:,yColumn);
...
k = 1; % which row in binsOfData to add next row to
b = 0; % which bin you're at
% Loop over all bins
for row = 1:size(allDataSorted, 1)
    tempRow = allDataSorted(row, :);
...
% meanBinCount = mean(binCount); % Print mean number of elements in the bins, for debugging
binMeans = binMeans(1:validRow - 1); % Extract valid means. We incremented validRow one too many times (see loop)
...
if(p.Results.subtractInitial)
    yPatches = yPatches - sensorData_FirstCell(p.Results.initialRow);
    binStatistics(:, 1) = binStatistics(:, 1) - sensorData_FirstCell(p.Results.initialRow);
end
