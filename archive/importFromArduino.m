...
%
%   Required inputs:
%   folder - the folder containing the file to be imported.
...
%   
%   Notes: 
%   1. All preceding arguments must be specified for a positional argument
%   to work. Ex: In order to have a start column, the startRow and endrow
%   must be specified.
%	2. Non-numeric cells are replaced with: NaN

%   TODO:
%   1. (Low Priority) Allow range to be specified by an array parameter.
%   Note that this requires some way to parse the array, which would have
%   both numeric and character entries.
%   2. (High Priority) Extend to allow csv files
...
addParameter(p,'sheetName',1,trueFunction)
